...
The model is composed by a prior network $B_0$ and a transition network $B^{t}_{t-1}$ for each transition $t-1 \rightarrow t$ in the case of a first-order DBN. A Transition is associated to a window $D^{t}_{t-1}$ composed by the observed attributes required to compute its corresponding anomaly score. A transition score is computed as
